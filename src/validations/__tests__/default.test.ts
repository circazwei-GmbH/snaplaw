import {length, lengthCheckIfAnotherFieldHasSpecificValue, lengthCheckIfAnotherFieldIsTrue} from "../default";
...
describe('lengthCheckIfAnotherFieldIsTrue', () => {
  it('Should return undefined on empty field', () => {
    const legntValidation = lengthCheckIfAnotherFieldIsTrue('error message', 1, 'test')
    expect(legntValidation('', {})).toEqual(undefined)
  })
  it('Should validate on enter field', () => {
    const legntValidation = lengthCheckIfAnotherFieldIsTrue('error message', 5, 'test')
...
describe('lengthCheckIfAnotherFieldHasSpecificValue', () => {
  it('Should return undefined on not valid income', () => {
    const lengthValidator = lengthCheckIfAnotherFieldHasSpecificValue('error message', 5, 'test', 'spValue')
  ...
  it('Should validate on match specific field', () => {
    const lengthValidator = lengthCheckIfAnotherFieldHasSpecificValue('error message', 5, 'test', 'spValue')
    expect(lengthValidator('test', {test: 'spValue'})).toEqual('error message')
