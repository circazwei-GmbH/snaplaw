...
const listActionConfig: ListActionType[] = [
  ...
  },
  {
    ...
    action: {
      ...
      handler: (item) => navigate({[MY_CONTRACT_ROUTE.PROFILE]: {id: item.partnerId}})
    }
  },
  {
    contractRole: CONTRACT_ROLE.OWNER,
...
export const getListItemAction = (item: ContractDataListType) => {
  return listActionConfig.filter((listAction) => {
    const currentStatus = item.partnerId ? CONTRACT_STATUS.WITH_PARTNER : CONTRACT_STATUS.WITHOUT_PARTNER;
    return item.meRole === listAction.contractRole && listAction.contractStatus.includes(currentStatus)
  })
}