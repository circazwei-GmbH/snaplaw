...
jest.mock("react-native-mime-types", () => ({
  lookup: jest.fn().mockImplementationOnce(() => 'image/png').mockImplementationOnce(() => 'video/mp4').mockImplementationOnce(() => 'test/mp4')
}))

global.fetch = jest.fn().mockImplementation((path) => {
  if (path === 'http://test-host/folder/name?data=type') {
    return {
      data: 'http://test-host/folder/name?data=type'
    }
  } else {
