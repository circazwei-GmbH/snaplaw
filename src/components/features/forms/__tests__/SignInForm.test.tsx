...
describe("SignInForm", () => {
  ...
  it("Should fields exists", async () => {
    ...
    expect(getByPlaceholderText("sign_in.email_field").props.value).toEqual(
      form.email.value
    );
    expect(getByPlaceholderText("sign_in.password_field")).toBeTruthy();
    expect(getByPlaceholderText("sign_in.password_field").props.value).toEqual(
      form.password.value
    );
  ...
  it("Should handler called", () => {
    ...
    fireEvent.changeText(
      getByPlaceholderText("sign_in.email_field"),
      TEST_EMAIL
    );
    expect(handler).toBeCalledWith("email", TEST_EMAIL);
    handler.mockClear();
    fireEvent.changeText(
      getByPlaceholderText("sign_in.password_field"),
      TEST_PASSWORD
    );
