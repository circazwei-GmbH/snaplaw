...
export default function RentalPrice() {
  ...
  const screenErrors = useAppSelector(
    (state) =>
      state.contract.contractErrors?.[CONTRACT_SCREEN_TYPES.RENTAL_PRICE]
  );
  const operatingCostsScreen = useAppSelector(
    (state) =>
      state.contract.currentContract?.screens.find(
        (screen) => screen.type === CONTRACT_SCREEN_TYPES.RENTAL_PRICE
  ...
  const updateDataHandler = (fieldName: PAYMENT_FIELDS, value: string) => {
    ...
    if (screenErrors?.[fieldName] && contractType) {
      dispatch(
        validateScreen(contractType, CONTRACT_SCREEN_TYPES.RENTAL_PRICE)
      );
    }
  };
  ...
  return (
    <View style={styles.container}>
      <PaymentPrice
        defaultText={t(
          `contracts.${contractType}.${CONTRACT_SCREEN_TYPES.RENTAL_PRICE}.product_price`
        )}
        cost={operatingCostsScreen?.data[PAYMENT_FIELDS.COST]}
        placeholder={t(
          `contracts.${contractType}.${CONTRACT_SCREEN_TYPES.RENTAL_PRICE}.fields.cost`
        )}
      ...
      <ErrorBoldMessage
        text={t(
          `contracts.${contractType}.${CONTRACT_SCREEN_TYPES.RENTAL_PRICE}.warning`
        )}
