...
describe('TextSwitch', () => {
  it('Should display left and right element', () => {
    const { getByText } = render(<TextSwitch left={LEFT} right={RIGHT} currentPosition={TEXT_SWITCH_POSITION.LEFT} onChange={jest.fn()} />)
    expect(getByText(LEFT)).toBeTruthy()
    expect(getByText(RIGHT)).toBeTruthy()
  })
  it('Should call handler on change', () => {
    const handler = jest.fn()
    const { getByText } = render(<TextSwitch left={LEFT} right={RIGHT} currentPosition={TEXT_SWITCH_POSITION.LEFT} onChange={handler} />)
