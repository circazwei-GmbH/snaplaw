...
jest.mock("../../features/Modals/ProductDescriptionModal", () => {
  ...
  return (props: any) => React.createElement('View', {...props, testID: "ModalMock"})
})

describe("DescriptionPhotos", () => {
  it("Should display photos", () => {
    const { getByTestId } = render(<DescriptionPhotos photos={PHOTOS_PROP} fieldName={PRODUCT_DESCRIPTION_FIELDS.accessoriesPhotos} />)
    expect(getByTestId("ImageTouchableWrapper")).toBeTruthy();
    expect(getByTestId("Image")).toBeTruthy()
    expect(getByTestId("Image").props.source).toEqual(buildMediaSource(PHOTOS_PROP[0]))
  })
  it("Should loading", () => {
    const { getByTestId, queryByTestId } = render(<DescriptionPhotos photos={PHOTOS_PROP} fieldName={PRODUCT_DESCRIPTION_FIELDS.accessoriesPhotos} />)
  ...
  it("Should call handler on delete", () => {
    ...
    const { getByTestId } = render(<DescriptionPhotos photos={PHOTOS_PROP} fieldName={PRODUCT_DESCRIPTION_FIELDS.accessoriesPhotos} onPressDelete={handler} />)
    expect(getByTestId("IamgeDeleteIcon")).toBeTruthy();
    fireEvent.press(getByTestId("IamgeDeleteIcon"))
    expect(handler).toBeCalledWith(PHOTOS_PROP[0], PRODUCT_DESCRIPTION_FIELDS.accessoriesPhotos)
  })
})